#include "kestrel_control/dynamic_camera_control_node.hpp"
...
All excess angle/motor control stuff is commented out so that it can be added in
later when we have to move the drone around
*/
...
DynamicCameraControlNode::DynamicCameraControlNode(
  ...
  camera_cmd_pub_ = this->create_publisher<kestrel_msgs::msg::CameraCommand>(
      "servo_camera_command", 5);

  /*
  drone_cmd_pub_ =
  this->create_publisher<std_msgs::msg::SomethingCommand>("drone_overshoot_command",
  5);
  */
...
void DynamicCameraControlNode::update_position_callback(
  ...
  double x_angle = x_pid_controller.compute_command(x_err, dt_sec);
  double y_angle = y_pid_controller.compute_command(y_err, dt_sec);
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::spin(std::make_shared<kestrel_control::DynamicCameraControlNode>());
  rclcpp::shutdown();
  return 0;
}
